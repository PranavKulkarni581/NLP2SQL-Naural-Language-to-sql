...
export default function LocalDb() {
  ...
  const convertAndExecute = async () => {
    ...
    try {
      ...
      const ee = (end - start).toFixed(2) + ' ms'
      setExecTime(ee)
      ...
      if (execData.success) {
        ...
        if (execData.rows && execData.rows.length > 0) {
          ...
          showToast('success', `Query executed successfully! (${ee})`)

        } else {
          // ✅ Non-SELECT (INSERT/UPDATE/DELETE)
          setRows([])
          const msg = `${execData.affectedRows || 0} rows affected. (${ee})`
          ...
          <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            ...
            <div className="flex flex-col gap-6">
              ...
              >
                ...
                {/* ✅ Show Execution Time */}
                {execTime && (
                  <p className="text-xs mt-1 text-indigo-600 dark:text-indigo-300 font-medium">
